...
# Kenneth Condon - University of Leicester - MSc Bioinformatics.
# 31st July 2016

# SETUP ##########################################################

# 1) Make a working directory and copy into it the following:
        # uppercase_remover.pl
        # all cuffdiff output files
        # merged.gtf file used in cuffdiff analysis

# 2) Run uppercase_remover.pl in the command line --> then return the script to your Perl tutorial scripts folder
...
setwd("[path to folder containing the files above")
getwd()

# 4) Create a cummbeRbund database (include genome and gtfFile arguments to annotate the database)
cuff <- readCufflinks(genome = 'rn6', gtfFile = "merged.gtf",rebuild = FALSE)
...
# To get a list of all gene names in the dataset
...
names<-featureNames(myGenes)          # gets all gene_short_names for each XLOC number
...
geneDiffExpLine<-expressionPlot(myGene) + labs(x="Blue = CONTROL | Red = TREATED",y="FPKM")
...
geneDiffExpBar<-expressionBarplot(myGene) + labs(x="Blue = CONTROL | Red = TREATED",y="FPKM")
...
head(fpkm(isoforms(myGenes)))                                     # remyturns all isoform_ids and their fpkm values for every gene from isoform_exp.diff (ignore sample_name column)

# To create a heatmap and bar plot of the gene set FPKM values for each condition (only use a bar plot when using simSet)
geneSetDiffExpHeat<-csHeatmap(myGenes,cluster='both')
